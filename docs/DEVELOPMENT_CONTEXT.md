...
- **ScriptRunnerService** (`src/modules/scripts/`) — FSM. Триггер по regex `intent[]`, последовательный сбор слотов (с опц. regex-валидацией), `confirm`-шаг, dispatch skill при подтверждении. Состояние в `Conversation.activeScript / activeScriptState / activeScriptSlots`. CANCEL/YES/NO детекторы используют explicit suffix `[\s,.!?]|$` (не `\b` — кириллица).
  - **Slot prefill** (`slots[].extract`): при активации скрипта значения слотов извлекаются из триггер-сообщения + последних реплик клиента (per-slot `extract` regex, capture-группа 1 или всё совпадение); извлечённое проходит `validate`, мусор отсекается. FSM переходит к первому НЕзаполненному слоту (или сразу к `confirm`, если всё известно). Решает «бот переспрашивает уже названное».
...
- **Папка на бизнес (per-business layout).** Всё, что принадлежит одному боту, лежит в `config/businesses/<id>/`: `configuration.json` (декларация бота, без скрытой сборки из нескольких файлов) + `data/<entity>.json` (структурные данные для skills). Цель — co-location: при переезде/клонировании бизнеса переносится одна папка, ничего не теряется. Резолв путей централизован в `src/modules/shared/config-paths.ts` (env-override `CONFIG_ROOT`); legacy-layout `config/configurations/<id>.json` + `config/data/<id>/` поддерживается как fallback для плавного перехода.
