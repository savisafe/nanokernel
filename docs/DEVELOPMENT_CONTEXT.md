...
- **Конфигурации бота** (`BotConfigurationModule`, глобальный модуль): один файл `config/configurations/<BOT_CONFIGURATION>.json` (переменная **`BOT_CONFIGURATION`**, по умолчанию `default`). В нём: **`llmPromptProfile`**, опционально **`useRag`**, опционально вложенный объект **`promptProfile`** (все поля промпта в том же файле). Примеры в репозитории: `knowledge-consultant`, `open-topics`.
...
- Переключение «какой бот запущен» — **`BOT_CONFIGURATION`** → один JSON в `config/configurations/` задаёт **`llmPromptProfile`**, опционально **`useRag`** и опционально весь промпт во вложенном **`promptProfile`**; **`LLM_PROMPT_PROFILE`** — запасной идентификатор профиля, если в JSON не задан `llmPromptProfile`.
