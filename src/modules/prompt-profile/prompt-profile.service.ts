...
import {
  PromptProfileFileJson,
  ResolvedLlmPromptProfile,
  RetrievalFusionConfig,
} from "./prompt-profile.types";
import {
...
export class PromptProfileService implements OnModuleInit {
  ...
  resolveFromPromptProfileJson(
    ...
    const retrievalFusion = this.parseRetrievalFusion(nestedRetrieval?.fusion);
  ...
  /** Разбирает блок retrieval.fusion в частичный конфиг; недостающие поля доберёт RagService. */
  private parseRetrievalFusion(
    raw: NonNullable<NonNullable<PromptProfileFileJson["retrieval"]>["fusion"]> | null | undefined,
  ): Partial<RetrievalFusionConfig> | undefined {
    if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
      return undefined;
    }
    const out: Partial<RetrievalFusionConfig> = {};
    const weightVector = this.parseFloatInRange(raw.weightVector, 0, 10);
    const weightLexical = this.parseFloatInRange(raw.weightLexical, 0, 10);
    const rrfK = this.parseIntInRange(raw.rrfK, 1, 1000);
    const poolMultiplier = this.parseIntInRange(raw.poolMultiplier, 1, 50);
    const poolMin = this.parseIntInRange(raw.poolMin, 1, 500);
    if (weightVector !== undefined) out.weightVector = weightVector;
    if (weightLexical !== undefined) out.weightLexical = weightLexical;
    if (rrfK !== undefined) out.rrfK = rrfK;
    if (poolMultiplier !== undefined) out.poolMultiplier = poolMultiplier;
    if (poolMin !== undefined) out.poolMin = poolMin;
